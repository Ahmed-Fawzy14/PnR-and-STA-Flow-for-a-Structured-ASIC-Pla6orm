...
class ClockTreeSynthesis:
    ...
    def load_logical_db(self, logical_db_file: str) -> None:
        ...
        for inst_name, inst_data in instances.items():
            ...
            is_sequential = (attrs.get("is_seq", False) or
                             any(k in cell_type for k in
                                 ("dff", "dfx", "dlat", "sdff", "flop", "ff_", "_ff", "dfbb", "dfrb", "dfrt")))
    ...
    def find_unused_buffers(self) -> None:
        ...
        print(
            f"[CTS] Total available buffers: {len(self.unused_buffers)} ({placed_count} placed-but-unused + {unplaced_count} unplaced)")
    ...
    def build_htree_recursive(self, sinks: List[str], level: int = 0) -> Optional[str]:
        ...
        if partition_by_x:
            ...
            sorted_sinks = sorted(valid_sinks,
                                  key=lambda s: self.get_sink_coordinate(s)[0] if self.get_sink_coordinate(s) else 0)
            ...
            sorted_sinks = sorted(valid_sinks,
                                  key=lambda s: self.get_sink_coordinate(s)[1] if self.get_sink_coordinate(s) else 0)
            mid = len(sorted_sinks) // 2
