...
Generate a gate-level Verilog netlist from:
  ...
  - build/<design>/<design>_logical_db.json  (for ports only)
...
Rules:
  ...
  - The top module is <design>_pd_eco with real ports taken from
    <design>_logical_db.json (ports + directions).
"""
...
def extract_ports_from_logical_db(logical_raw) -> Dict[str, dict]:
    ...
    Returns the 'ports' dict as-is (with a bit of sanity checking).
    """
    ...
    for pname, pinfo in ports.items():
        ...
        if "direction" not in pinfo:
            # Default to input if missing, just to be robust
...
def remap_pin_name_for_cell(cell_type: str, pin_name: str) -> str:
    ...
    For Sky130 buffers, we want the output pin to be X instead of Y.
    """
    ct_upper = cell_type.upper()

    # Any buffer-like cell whose output is currently called 'Y' in our db
...
def collect_all_net_bits(instances: Dict[str, dict], ports: Dict[str, dict]) -> Set[int]:
    """
...
def generate_verilog(
    ...
    Also builds a real top module interface from ports:
      module <design>_pd_eco (port1, port2, ...);
        input  ...;
        output ...;
        ...
    """
    ...
    # 3) collect nets (instances + ports)
    net_bits = collect_all_net_bits(instances, ports)
    ...
    for pname in port_names:
        ...
        # direction is typically "input", "output", "inout"
        dir_kw = direction.lower()
        if dir_kw not in ("input", "output", "inout"):
            dir_kw = "input"  # fallback
    ...
    for pname in port_names:
        ...
        if width == 1:
            b0 = bits[0]
            if direction == "input":
                # External drives port, port drives internal net
                lines.append(f"  assign {net_name(b0)} = {pname};")
            elif direction in ("output", "inout"):
                # Internal net drives port
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "from data_structures.json, mapped_netlist_graph_after_pd_eco, "
            "and logical_db (for ports)."
        )
    )
    parser.add_argument(
        "--design",
    ...
    verilog_src = generate_verilog(design, instances, graph_after, ports)
