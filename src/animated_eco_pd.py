...
def animate_pd_eco(
    ...
    # ============================================================
    # SIZE CONTROL 1: limit how many spares we animate
    # ============================================================
    total_spares = len(spare_pts)
    MAX_SPARES_TO_ANIMATE = 300
    ...
    if total_spares > MAX_SPARES_TO_ANIMATE:
        ...
        spare_pts = spare_pts[::step]
        total_spares = len(spare_pts)

    # ============================================================
    # SIZE CONTROL 2: limit frames per spare + overall frames
    # ============================================================
    FRAMES_PER_SPARE = 3
    MAX_FRAMES = 900  # 300 spares * 3 frames/spare

    total_frames = total_spares * FRAMES_PER_SPARE
    if total_frames > MAX_FRAMES:
        FRAMES_PER_SPARE = max(1, MAX_FRAMES // max(1, total_spares))
        total_frames = total_spares * FRAMES_PER_SPARE

    print(f"[VIS] Animation complexity (after caps): "
          f"{total_spares} spares, {FRAMES_PER_SPARE} frames/spare, "
          f"{total_frames} total frames")

    # ============================================================
    # SIZE CONTROL 3: keep big fabric figure, but lower DPI
    # ============================================================
    GIF_FIGSIZE = (10, 10)  # back to original "big" canvas
    GIF_FPS = 10
    GIF_DPI = 80            # still lower than default to keep file small
    ...
    # Text overlays
    # -----------------------------
    # text_info back to original style: data coordinates over the fabric
    text_info = ax.text(
        650, 1010, "",
        ha="right", va="top",
        fontsize=8,
    ...
    # counter stays in axes coordinates in the corner
    ...
    anim.save(out_gif_path, writer="pillow", fps=GIF_FPS, dpi=GIF_DPI)
    plt.close(fig)
