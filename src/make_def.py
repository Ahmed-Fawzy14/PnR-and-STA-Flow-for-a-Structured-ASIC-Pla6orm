...
def load_fabric_cells(fabric_cells_path: str, template_map: Dict[str, str], data_path: Optional[str] = None) -> Dict[str, Dict[str, Any]]:
    """
    ...
    for tile_name, tile_details in tiles.items():
        ...
        for cell in cells:
            ...
            if not physical_cell_type:
                print(f"[WARNING] No physical cell type for template '{template_name}' in cell '{cell_name}'. Skipping.")
                continue
...
def calculate_die_area(slot_info: Dict[str, Dict[str, Any]], margin_um: float = 5.0) -> Tuple[float, float, float, float]:
    """
...
def write_def_file(
    ...
    with open(output_path, 'w') as f:
        ...
        for pin in pins:
            ...
            f.write(f"  - {pin_name} + NET {pin_name} + DIRECTION {def_direction} + USE SIGNAL + LAYER met2 ( -140 0 ) ( 140 280 ) + FIXED ( {x_dbu} {y_dbu} ) N ;\n")
        f.write("END PINS\n")
        f.write("\n")
...
def main():
    ...
    if fabric_db_path:
        ...
        slot_info = load_fabric_cells(fabric_cells_path, template_map, data_path=data_path if os.path.exists(data_path) else None)
    ...
    if not pins and os.path.exists(data_path):
        try:
            ...
            pins = get_pins_from_data(data, logical_db_path=logical_db_path if os.path.exists(logical_db_path) else None)
            if pins:
