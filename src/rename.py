...
def load_map(map_path: str) -> Dict[str, str]:
    """
    Load a <orig_inst> <slot> map file into a dictionary: orig_inst_name -> slot_name.
    Ignores blank lines and lines starting with '#'.

    We also add a second key with a leading backslash for tools that
    escape strange identifiers in Verilog (e.g. \foo.bar).
    """
    ...
    with open(map_path, "r", encoding="utf-8") as f:
        for lineno, line in enumerate(f, start=1):
            ...
            orig_inst, slot = parts  # col1 = old inst, col2 = new slot

            # Key 1: raw name from the map
            if orig_inst in mapping and mapping[orig_inst] != slot:
                raise ValueError(
                    f"Instance {orig_inst!r} appears multiple times in map with "
                    f"different slots: {mapping[orig_inst]!r} vs {slot!r}"
                )
            mapping[orig_inst] = slot

            # Key 2: escaped version with leading backslash, in case the Verilog
            # wrote it as an escaped identifier like \foo.bar
            if not orig_inst.startswith("\\"):
                escaped = "\\" + orig_inst
                if escaped not in mapping:
                    mapping[escaped] = slot
...
def rename_instances_in_verilog(
    ...
    inst_pattern = re.compile(
        ...
        r'(\S+)'  # 2: cell type (any non-whitespace)
        r'\s+'  # space(s)
        r'(\S+)'  # 3: instance name (any non-whitespace)
        r'(\s*(?:#\s*\(|\())'  # 4: " (" or " #("
    )
